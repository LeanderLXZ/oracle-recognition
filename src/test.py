...
class Test(object):
  ...
  def __init__(self, cfg):
    ...
    # Predict multi-objects
    self.pred_multi_obj = False
  ...
  def _get_tensors(self, loaded_graph):
    ...
    with loaded_graph.as_default():
      ...
      preds_ = loaded_graph.get_tensor_by_name("logits:0")
      loss_ = loaded_graph.get_tensor_by_name("preds:0")
  ...
  def _eval(self,
    ...
    return loss_, clf_loss_, rec_loss_, acc_, preds_
  ...
  def _get_multi_obj_eval(self, preds_vec, **kwargs):
    """Get evaluation for multi-objects detection."""
  ...
  def test(self):
    ...
    with tf.Session(graph=loaded_graph) as sess:
      ...
      if self.cfg.TEST_WITH_RECONSTRUCTION:
        inputs, labels, preds, loss, accuracy, \
            clf_loss, rec_loss, rec_images = \
            self._get_tensors(loaded_graph)
      else:
        inputs, labels, preds, loss, accuracy = self._get_tensors(loaded_graph)
      ...
      if self.cfg.TEST_BATCH_SIZE:
        loss_test, clf_loss_test, rec_loss_test, acc_test, preds_vec_test = \
            self._eval_on_batches(
                sess, inputs, labels, preds, loss, accuracy,
                clf_loss, rec_loss, rec_images,
                self.x_test, self.y_test)
      else:
        loss_test, clf_loss_test, rec_loss_test, acc_test, preds_vec_test = \
          self._eval(sess, inputs, labels, preds, loss, accuracy, clf_loss,
                     rec_loss, rec_images, self.x_test, self.y_test)

      if self.pred_multi_obj:
        precision, recall, accuracy, f1score = \
          self._get_multi_obj_eval(preds_vec_test,
                                   predict_mode='top_n',
                                   predict_num=2)
        acc_test = accuracy
...
class TestMultiObjects(Test):
  ...
  def _get_tensors(self, loaded_graph):
    ...
    with loaded_graph.as_default():
      ...
      inputs_ = loaded_graph.get_tensor_by_name("inputs:0")
      labels_ = loaded_graph.get_tensor_by_name("labels:0")
      logits_ = loaded_graph.get_tensor_by_name("logits:0")

      if self.cfg.TEST_WITH_RECONSTRUCTION:
        clf_loss_ = loaded_graph.get_tensor_by_name("classifier_loss:0")
        rec_loss_ = loaded_graph.get_tensor_by_name("rec_loss:0")
        rec_images_ = loaded_graph.get_tensor_by_name("rec_images:0")
        return inputs_, labels_, logits_, \
            clf_loss_, rec_loss_, rec_images_
      else:
        return inputs_, labels_, logits_
  ...
  def _eval(self,
    ...
    """Evaluation."""
    if self.cfg.TEST_WITH_RECONSTRUCTION:
      loss_, clf_loss_, rec_loss_, acc_, preds_ = \
        sess.run([loss, clf_loss, rec_loss, accuracy, preds],
                 feed_dict={inputs: x, labels: y})
      # Save reconstruct images
      if self.cfg.TEST_SAVE_IMAGE_STEP:
        self._save_images(sess, rec_images, inputs, labels, x, y)
